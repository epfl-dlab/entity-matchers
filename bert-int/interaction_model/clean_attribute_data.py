...
def read_att_data_entity_matchers(data_path):
    print("loading attribute triples file from: ", data_path)
    att_data = []
    with open(data_path, "r", encoding="utf-8") as f:
        for line in f:
            e, a, l = line.rstrip('\n').split("\t")
            a = a.split(r'/')[-1] # Take after last / (checked is good for all our datasets)
            l = "@".join(l.split("@")[:-1])
            if len(l.rsplit('^^', 1)) == 2:
                l, l_type = l.rsplit("^^")
            else:
...
def main(args, swap=False):
    ...
    if args is None:
        ...
        keep_data_1 = read_att_data_entity_matchers(args.dataset + '/attr_triples_1')
        keep_data_2 = read_att_data_entity_matchers(args.dataset + '/attr_triples_2')

    if swap:
        keep_data_1, keep_data_2 = keep_data_2, keep_data_1
    ...
    remove_attribute_triple_1_file_path = INTER_PATH + 'remove_att_triples_1_{}'.format(dataset_prefix)
    remove_attribute_triple_2_file_path = INTER_PATH + 'remove_att_triples_2_{}'.format(dataset_prefix)
