...
def test(Model, test_candidate, test_ill, entpair2f_idx, f_emb, batch_size, cuda_num, test_topk, ent_out_1):
    ...
    for (e1, e2_list) in test_candidate.items():
        for e2 in e2_list:
            test_pairs.append((e1, e2))
...
def train(Model, Optimizer, Criterion, Train_gene, f_emb_list, test_candidate, test_ill,
          entpair2f_idx, ent_out_1, epoch_num, eval_num, cuda_num, test_topk):
    ...
    for epoch in range(epoch_num):
        ...
        if (epoch + 1) % eval_num == 0 and epoch != 0:
            start_time = time.time()
            best_pairs = test(Model, test_candidate, test_ill, entpair2f_idx, feature_emb, 2048,
                              cuda_num, test_topk, ent_out_1)
