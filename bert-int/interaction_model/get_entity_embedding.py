...
def main(args):
    ...
    bert_model_path = BASIC_BERT_UNIT_MODEL_SAVE_PATH + dataset_prefix + "model_epoch_" \
    ...
    pickle.dump(ent_emb, open(ENT_EMB_PATH.format(dataset_prefix, LOAD_BASIC_BERT_UNIT_MODEL_EPOCH_NUM), "wb"))
    ...
    ent_out_1, ent_out_2 = pickle.load(open(save_out_ent_path, "rb"))

    test_ids_1 = [e1 for e1, e2 in test_ill] + ent_out_1
    test_ids_2 = [e2 for e1, e2 in test_ill] + ent_out_2
    train_ids_1 = [e1 for e1, e2 in train_ill]
    train_ids_2 = [e2 for e1, e2 in train_ill]
    train_candidates = candidate_generate(train_ids_1, train_ids_2, ent_emb, CANDIDATE_NUM, bs=2048, cuda_num=args.gpu)
    test_candidates = candidate_generate(test_ids_1, test_ids_2, ent_emb, CANDIDATE_NUM, bs=2048, cuda_num=args.gpu)
    assert len(test_candidates) == len(test_ill) + len(ent_out_1)
    pickle.dump(train_candidates, open(TRAIN_CANDIDATES_PATH.format(dataset_prefix), "wb"))
    print("save candidates for training ILL data....")
    pickle.dump(test_candidates, open(TEST_CANDIDATES_PATH.format(dataset_prefix), "wb"))
    ...
    pickle.dump(entity_pairs, open(ENT_PAIRS_PATH.format(dataset_prefix), "wb"))
