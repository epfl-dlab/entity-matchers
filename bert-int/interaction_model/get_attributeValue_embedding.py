...
def main(args):
    ...
    if args is None:
        ...
        ent_ill, index2rel, index2entity, \
        rel2index, entity2index, \
        rel_triples_1, rel_triples_2 = pickle.load(open(save_read_data_path, "rb"))

    # load attribute triples files.
    ents = [ent for ent in entity2index.keys()]
    new_attribute_triple_1_file_path = INTER_PATH + 'new_att_triples_1_{}'.format(dataset_prefix)
    new_attribute_triple_2_file_path = INTER_PATH + 'new_att_triples_2_{}'.format(dataset_prefix)
    ...
    # TODO: changed add_name_as_attTriples to false in our more realistic setup
    ...
    pickle.dump(value_emb, open(ATTRIBUTEVALUE_EMB_PATH.format(dataset_prefix), "wb"))
    pickle.dump(value_set, open(ATTRIBUTEVALUE_LIST_PATH.format(dataset_prefix), "wb"))
    print("save attributeValue embedding in: ", ATTRIBUTEVALUE_EMB_PATH.format(dataset_prefix))
    print("save attributeValue list in: ", ATTRIBUTEVALUE_LIST_PATH.format(dataset_prefix))
