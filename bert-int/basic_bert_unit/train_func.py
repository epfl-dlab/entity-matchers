...
def generate_candidate_dict(Model, train_ent1s, train_ent2s, for_candidate_ent1s, for_candidate_ent2s,
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(train_ent1s), batch_size):
            temp_emb = entlist2emb(Model, train_ent1s[i:i + batch_size], entid2data, args.gpu).cpu().tolist()
            train_emb1.extend(temp_emb)
        for i in range(0, len(for_candidate_ent2s), batch_size):
            temp_emb = entlist2emb(Model, for_candidate_ent2s[i:i + batch_size], entid2data, args.gpu).cpu().tolist()
        ...
        for i in range(0, len(train_ent2s), batch_size):
            temp_emb = entlist2emb(Model, train_ent2s[i:i + batch_size], entid2data, args.gpu).cpu().tolist()
            train_emb2.extend(temp_emb)
        for i in range(0, len(for_candidate_ent1s), batch_size):
            temp_emb = entlist2emb(Model, for_candidate_ent1s[i:i + batch_size], entid2data, args.gpu).cpu().tolist()
...
def train(Model, Criterion, Optimizer, Train_gene, train_ill, test_ill, entid2data, args):
    ...
    for epoch in range(EPOCH_NUM):
        ...
        candidate_dict = generate_candidate_dict(Model, train_ent1s, train_ent2s, for_candidate_ent1s,
                                                 for_candidate_ent2s, entid2data, Train_gene.index2entity, args)
        Train_gene.train_index_gene(candidate_dict)  # generate training data with candidate_dict

        # train
        epoch_loss, epoch_train_time = ent_align_train(Model, Criterion, Optimizer, Train_gene, entid2data, args)
        Optimizer.zero_grad()
        torch.cuda.empty_cache()
        print("Epoch {}: loss {:.3f}, using time {:.3f}".format(epoch, epoch_loss, epoch_train_time))
        if epoch >= 0:
            if epoch != 0:
                save(Model, train_ill, test_ill, entid2data, epoch, args)
            # test(Model,train_ill,entid2data,TEST_BATCH_SIZE,context="EVAL IN TRAIN SET")
            test(Model, test_ill, entid2data, args.test_batch_size_basic, args, context="EVAL IN TEST SET:")


def test(Model, ent_ill, entid2data, batch_size, args, context=""):
    print("-----test start-----")
    start_time = time.time()
    print(context)
    Model.eval()
    with torch.no_grad():
        ...
        for i in range(0, len(ents_1), batch_size):
            ...
            batch_emb_1 = entlist2emb(Model, batch_ents_1, entid2data, args.gpu).detach().cpu().tolist()
        ...
        for i in range(0, len(ents_2), batch_size):
            ...
            batch_emb_2 = entlist2emb(Model, batch_ents_2, entid2data, args.gpu).detach().cpu().tolist()
            emb2.extend(batch_emb_2)
            del batch_emb_2

        print("Cosine similarity of basic bert unit embedding res:")
        res_mat = cos_sim_mat_generate(emb1, emb2, batch_size, cuda_num=args.gpu)
        score, top_index = batch_topk(res_mat, batch_size, topn=TOPK, largest=True, cuda_num=args.gpu)
        hit_res(top_index)
    print("test using time: {:.3f}".format(time.time() - start_time))
    print("--------------------")


def save(Model, train_ill, test_ill, entid2data, epoch_num, args):
    save_prefix = args.dataset.split("/")[-1]
    print("Model {} save in: ".format(epoch_num),
          MODEL_SAVE_PATH + save_prefix + "model_epoch_" + str(epoch_num) + '.p')
    Model.eval()
    torch.save(Model.state_dict(), MODEL_SAVE_PATH + save_prefix + "model_epoch_" + str(epoch_num) + '.p')
    other_data = [train_ill, test_ill, entid2data]
    pickle.dump(other_data, open(MODEL_SAVE_PATH + save_prefix + 'other_data.pkl', "wb"))
    print("Model {} save end.".format(epoch_num))


def ent_align_train(Model, Criterion, Optimizer, Train_gene, entid2data, args):
    start_time = time.time()
    all_loss = 0
    Model.train()
    for pe1s, pe2s, ne1s, ne2s in Train_gene:
        Optimizer.zero_grad()
        pos_emb1 = entlist2emb(Model, pe1s, entid2data, cuda_num=args.gpu)
        pos_emb2 = entlist2emb(Model, pe2s, entid2data, cuda_num=args.gpu)
        batch_length = pos_emb1.shape[0]
        pos_score = F.pairwise_distance(pos_emb1, pos_emb2, p=1, keepdim=True)  # L1 distance
        del pos_emb1
        del pos_emb2

        neg_emb1 = entlist2emb(Model, ne1s, entid2data, cuda_num=args.gpu)
        neg_emb2 = entlist2emb(Model, ne2s, entid2data, cuda_num=args.gpu)
        neg_score = F.pairwise_distance(neg_emb1, neg_emb2, p=1, keepdim=True)
        del neg_emb1
        del neg_emb2

        label_y = -torch.ones(pos_score.shape).cuda(args.gpu)  # pos_score < neg_score
