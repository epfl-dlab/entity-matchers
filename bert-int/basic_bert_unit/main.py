...
from .Read_data_func import read_data, read_dataset_entity_matchers
from .Param import *
...
def main(args, swap=False):
    ...
    if args is None:
        ...
        ent_out1, ent_out2 = read_dataset_entity_matchers(args.dataset, args.fold_folder,
                                                          args.dict_path, swap)
    ...
    Criterion = nn.MarginRankingLoss(args.margin_basic, size_average=True)
    Optimizer = AdamW(Model.parameters(), lr=args.lr_basic)

    ent1 = [e1 for e1, e2 in ent_ill] + ent_out1
    ent2 = [e2 for e1, e2 in ent_ill] + ent_out2

    # training data generator(can generate batch-size training data)
    Train_gene = Batch_TrainData_Generator(train_ill, ent1, ent2, index2entity, batch_size=args.train_batch_size_basic,
                                           neg_num=args.neg_num_basic)

    train(Model, Criterion, Optimizer, Train_gene, train_ill, test_ill, ent2data, args)
