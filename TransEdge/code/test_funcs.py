...
def valid_no_csls(embed1, embed2, ids_1, ids_2, ppre_f1, pre_f1, nums_threads, valid_links):
    sim_mat = sim_handler(embed1, embed2, 0, nums_threads)
    align_kg1_kg2 = np.argmax(sim_mat, axis=1)  # Get the max id on the rows
    align_kg2_kg1 = np.argmax(sim_mat, axis=0)  # Get the max id on the columns
    aligns = compute_bidirectional_alignments(align_kg1_kg2, align_kg2_kg1, ids_1, ids_2)
    prec, rec, f1 = compute_prec_rec_f1(aligns, valid_links)
    print("Validation step")
    print("Precision:", prec)
    print("Recall:", rec)
    print("F1:", f1)
    ppre_f1, pre_f1, is_early = early_stop(ppre_f1, pre_f1, f1)
    return ppre_f1, pre_f1, is_early

def test_final(embed1, embed2, ids_1, ids_2, nums_threads, test_links, csls):
    sim_mat = sim_handler(embed1, embed2, 0, nums_threads)
    align_kg1_kg2 = np.argmax(sim_mat, axis=1)  # Get the max id on the rows
    align_kg2_kg1 = np.argmax(sim_mat, axis=0)  # Get the max id on the columns
    aligns = compute_bidirectional_alignments(align_kg1_kg2, align_kg2_kg1, ids_1, ids_2)
    prec, rec, f1 = compute_prec_rec_f1(aligns, test_links)
...
def CSLS_sim(sim_mat1, k, nums_threads):
    ...
    if sim_mat1.shape[0] < 50000:
        ...
        print("Calculating CSLS values in one thread...")
        sim_values = cal_csls_sim(sim_mat1, k)
