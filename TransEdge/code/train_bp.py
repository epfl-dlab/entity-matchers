...
def bootstrapping(ref_sim_mat, ref_ent1, ref_ent2, labeled_alignment, th, top_k, len_valid_test, is_edit=False):
    # TODO: is_edit is True for BootEA/HyperKA, check what may happen
    n = ref_sim_mat.shape[0]
    curr_labeled_alignment = find_potential_alignment(ref_sim_mat, th, top_k, n, len_valid_test)
    if is_edit:
        if curr_labeled_alignment is not None:
            labeled_alignment = update_labeled_alignment(labeled_alignment, curr_labeled_alignment, ref_sim_mat, n, len_valid_test)
            labeled_alignment = update_labeled_alignment_label(labeled_alignment, ref_sim_mat, n, len_valid_test)
...
def check_alignment(aligned_pairs, all_n, len_valid_test, context="", is_cal=True):
    if aligned_pairs is None or len(aligned_pairs) == 0:
        print("{}, Empty aligned pairs".format(context))
        return
    num = 0
    for x, y in aligned_pairs:
        if x == y and x < len_valid_test:
            num += 1
...
def find_potential_alignment(sim_mat, sim_th, k, total_n, len_valid_test):
    t = time.time()
    potential_aligned_pairs = generate_alignment(sim_mat, sim_th, k, total_n, len_valid_test)
    ...
    selected_pairs = mwgm(potential_aligned_pairs, sim_mat, mwgm_graph_tool)
    check_alignment(selected_pairs, total_n, len_valid_test, context="selected_pairs")
...
def generate_alignment(sim_mat, sim_th, k, all_n, len_valid_test):
    potential_aligned_pairs = filter_mat(sim_mat, sim_th)
    if len(potential_aligned_pairs) == 0:
        return None
    check_alignment(potential_aligned_pairs, all_n, len_valid_test, context="after sim filtered")
    neighbors = search_nearest_k(sim_mat, k)
    if neighbors is not None:
        potential_aligned_pairs &= neighbors
        if len(potential_aligned_pairs) == 0:
            return None, None
        check_alignment(potential_aligned_pairs, all_n, len_valid_test, context="after sim and neighbours filtered")
    del neighbors
...
def update_labeled_alignment(labeled_alignment, curr_labeled_alignment, sim_mat, all_n, len_valid_test):
    ...
    for i, j in curr_labeled_alignment:
        ...
        if i in labeled_alignment_dict.keys():
            ...
            new_sim = sim_mat[i, j]
            if new_sim >= old_sim:
                if jj == i and j != i and i < len_valid_test:
                    n += 1
    ...
    check_alignment(labeled_alignment, all_n, len_valid_test, context="after editing labeled alignment (<-)")
...
def update_labeled_alignment_label(labeled_alignment, sim_mat, all_n, len_valid_test):
    ...
    check_alignment(updated_alignment, all_n, len_valid_test, context="after editing labeled alignment (->)")
