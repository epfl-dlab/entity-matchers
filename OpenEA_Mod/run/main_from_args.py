...
# Avoid tensorflow to use all resourcers -> may need to add this in the source code inside load_session
...
if __name__ == '__main__':
    ...
    args.dataset_division = sys.argv[3]      # This is the fold used among the k (5) available.
    if len(sys.argv) < 5:
        args.gpu = "0"
    else:
        if sys.argv[4] == "CPU":
    ...
    kgs = read_kgs_from_folder(args.training_data, args.dataset_division, args.alignment_module, args.ordered,
                               args.extra_entities_percentage_valid, remove_unlinked=remove_unlinked)
    model = get_model(args.embedding_module)()  # Get the current model class from args
    model.set_args(args)
    model.set_kgs(kgs)
    model.init()
    model.run()
