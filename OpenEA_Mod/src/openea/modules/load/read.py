...
def generate_mapping_id(kg1_triples, kg1_elements, kg2_triples, kg2_elements, ordered=True):
    # note elements is entities/relations/attributes
    ids1, ids2 = dict(), dict()
    if ordered:
        # sort elements by frequency, then by keys (IDs) -> note this function return order of anything
        # it depends on the elements we pass (so entities, relations, attributes (properties))
        ...
        # generate ids and put in a dict -> id[element (can be whatever)] = val
        # once finished one list put just element from the other list
        for i in range(n):
            if i < n1 and i < n2:
        ...
        # not sorted case: just put different ids for different entities
        index = 0
        for ele in kg1_elements:
            if ele not in ids1:
                ids1[ele] = index
...
def uris_relation_triple_2ids(uris, ent_ids, rel_ids):
    # convert the triples in triples of ids
    id_uris = list()
...
def uris_attribute_triple_2ids(uris, ent_ids, attr_ids):
    id_uris = list()
    # convert triples in (id_ent, id_prop, literal)
    for u1, u2, u3 in uris:
        assert u1 in ent_ids
...
def generate_sup_relation_triples_one_link(e1, e2, rt_dict, hr_dict):
    new_triples = set()
    # add e2 as head of all the relations for which e1 is head
    for r, t in rt_dict.get(e1, set()):
        new_triples.add((e2, r, t))
    # add e2 as tail for all the relations in which e1 is tail
...
def generate_sup_attribute_triples(sup_links, av_dict1, av_dict2):
    new_triples1, new_triples2 = set(), set()
    # add all literals of e2 to e1 and viceversa
    for ent1, ent2 in sup_links:
...
def read_relation_triples(file_path: str):
    """
    Return three sets containing: the triples (h, r, t), the entities and the relations
    parsed from the file named file_path.
    Parameters
    ----------
    file_path: str the name of the file path for the relation (for example, .../attr_triple_1).

    Returns
    -------
    triples a set of 3-tuples h, r, t.
    entities a set with all entities (both h's and t's).
    relations a set with all relations.
    """
    ...
    # generate tuples (h,r,t) in triples, entities contains all the h and t, relations all the r
...
def read_links(file_path: str):
    """
    Links are pairs with entity from kg1 \t entity from kg2
    and represent the ground truth.
    Parameters
    ----------
    file_path: str of the file path with the ground truth

    Returns
    -------
    links: list of 2-tuples (e1, e2) where e1 belongs to kg1,
            e2 belongs to kg1 and their link is the so called ground truth.
    """
...
def save_embeddings(folder, kgs, ent_embeds, rel_embeds, attr_embeds, mapping_mat=None, rev_mapping_mat=None,
    ...
    if iteration is None:
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        if ent_embeds is not None:
            np.save(folder + 'ent_embeds_' + str(iteration) + '_.npy', ent_embeds)
        if rel_embeds is not None:
            np.save(folder + 'rel_embeds_' + str(iteration) + '_.npy', rel_embeds)
        if attr_embeds is not None:
            np.save(folder + 'attr_embeds_' + str(iteration) + '_.npy', attr_embeds)
        if mapping_mat is not None:
            np.save(folder + 'mapping_mat_' + str(iteration) + '_.npy', mapping_mat)
        if rev_mapping_mat is not None:
            np.save(folder + 'rev_mapping_mat_' + str(iteration) + '_.npy', rev_mapping_mat)
        print("Embeddings saved for iteration " + str(iteration) + "!")


def read_attribute_triples(file_path: str):
    """
    Parse the attribute triples from the file specified, and return three sets
    with the triples, entities and attributes.
    Parameters
    ----------
    file_path: str with the name of the file (like .../attr_triples_1)

    Returns
    -------
    triples: set of (head, attr, value) tuples
    entities: set of head elems
    ...
    file = open(file_path, 'r', encoding='utf8')
    # triples contains (entity, prop, literal), entities all entities, attributes all literals
    for line in file.readlines():
