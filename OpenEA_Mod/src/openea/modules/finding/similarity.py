...
def sim(embed1, embed2, metric='inner', normalize=False, csls_k=0, debug = False):
    """
    ...
    if metric == 'inner':
        ...
        if debug:
            print("Saving matrix no csls BootEA")
            print("sim_mat shape", sim_mat.shape)
            if sim_mat.shape == (1500, 10500):
                with open("sim_mat_train_no_csls_BootEA.npy", "wb") as f:
                    np.save(f, sim_mat)
            else:
                with open("sim_mat_test_no_csls_BootEA.npy", "wb") as f:
                    np.save(f, sim_mat)
        ...
        sim_mat = sim_mat.astype(np.float32)
        if debug:
            print("Saving matrix no csls RDGCN")
            print("sim_mat shape", sim_mat.shape)
            if sim_mat.shape == (1500, 10500):
                with open("sim_mat_train_no_csls_RDGCN.npy", "wb") as f:
                    np.save(f, sim_mat)
            else:
                with open("sim_mat_test_no_csls_RDGCN.npy", "wb") as f:
                    np.save(f, sim_mat)
    else:
    ...
    if csls_k > 0:
        ...
        if debug:
            if metric == "inner":
                print("Saving matrix csls BootEA")
                if sim_mat.shape == (1500, 10500):
                    with open("sim_mat_train_csls_BootEA.npy", "wb") as f:
                        np.save(f, sim_mat)
                else:
                    with open("sim_mat_test_csls_BootEA.npy", "wb") as f:
                        np.save(f, sim_mat)
            elif metric=="manhattan":
                print("Saving matrix csls RDGCN")
                if sim_mat.shape == (1500, 10500):
                    with open("sim_mat_train_csls_RDGCN.npy", "wb") as f:
                        np.save(f, sim_mat)
                else:
                    with open("sim_mat_test_csls_RDGCN.npy", "wb") as f:
                        np.save(f, sim_mat)
    return sim_mat
