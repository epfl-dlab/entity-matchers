...
def find_potential_alignment_mwgm(sim_mat, sim_th, k, len_valid_test, heuristic=True):
    t = time.time()
    potential_aligned_pairs = find_alignment(sim_mat, sim_th, k, len_valid_test)
    ...
    check_new_alignment(selected_aligned_pairs, len_valid_test, context="after mwgm")
...
def find_alignment(sim_mat, sim_th, k, len_valid_test):
    """
    ...
    if len(potential_aligned_pairs) == 0:
        return None
    check_new_alignment(potential_aligned_pairs, len_valid_test, context="after filtering by sim threshold")
    if k <= 0:
    ...
    if len(potential_aligned_pairs) == 0:
        return None
    check_new_alignment(potential_aligned_pairs, len_valid_test, context="after filtering by sim and nearest k")
    return potential_aligned_pairs
...
def mwgm_graph_tool(pairs, sim_mat):
    # Perform a max cardinality matching in a graphes whose nodes are the x,y pairs and there is an edge between each
    # Return the ones for which the edge has a value 1
...
def check_new_alignment(aligned_pairs, len_valid_test, context="check alignment"):
    if aligned_pairs is None or len(aligned_pairs) == 0:
        print("{}, empty aligned pairs".format(context))
        return
    num = 0
    # If it's the index in the diagonal!!
    for x, y in aligned_pairs:
        if x == y and x < len_valid_test:  # If x >= len_valid_test is an outside entities match, so it's wrong
            num += 1
