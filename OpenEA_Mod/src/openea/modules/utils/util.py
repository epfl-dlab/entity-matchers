...
def load_session(gpu="0"):
    # Configure Tensorflow session (TODO: may need to change it for our project)
    gpu_options = tf.GPUOptions(visible_device_list=gpu, allow_growth=True)
...
def task_divide(idx, n):
    # Choose how to divide the tasks between different threads, depending on thread number n
    total = len(idx)
    if n <= 0 or 0 == total:
        return [idx]
    if n > total:
...
def generate_out_folder(out_folder, training_data_path, div_path, method_name):
    # Create output folder using parameters defined format out/methodname/dataset/fold/time
