...
from openea.modules.finding.evaluation import valid, test, early_stop, compute_prec_rec_f1
...
from openea.modules.finding.alignment import stable_alignment, compute_best_alignment_one_side, \
    compute_bidirectional_alignments, compute_top_k_avg_csls, compute_best_alignment_csls
import torch
...
class BasicModel:
    ...
    def set_args(self, args):
        ...
        if torch.cuda.is_available() and self.args.gpu is not None:
            self.torch_device = "cuda:" + self.args.gpu
        else:
    ...
    def _eval_valid_embeddings(self):
        # if we have validation use validation, otherwise tests
        if len(self.kgs.valid_links) > 0:
            # get the embeddings by evaluation on their graph
        ...
        # note BootEA doesn't use mapping so returns directly embeddings from lookup
    ...
    def launch_triple_training_1epo(self, epoch, triple_steps, steps_tasks, batch_queue, neighbors1, neighbors2):
        ...
        for steps_task in steps_tasks:
            # Generate a process that will do the training for the task,
            # launching bat.generate_relation_triple_batch_queue
        ...
        for i in range(triple_steps):
            ...
            # run in tensorflow, substituting the optimized value for the embeddings inside the triples
            # return batch_loss as first value, don't care about the optimizer fetch
            # note that triple_loss was defined in AlignE and is the one from eq.3 in BootEA paper
        ...
        # shuffle for next round of iteration
    ...
    def valid_new(self, stop_metric_new, method, ids_1, ids_2, embeds1, embeds2):
        embeds1 = torch.from_numpy(embeds1).float().to(self.torch_device)
        embeds2 = torch.from_numpy(embeds2).float().to(self.torch_device)
        ...
        prec, rec, f1 = compute_prec_rec_f1(aligns, self.kgs.valid_links)
        print("Validation step:")
        print("\tPrecision:", prec)
        print("\tRecall:", rec)
        print("\tF1:", f1)
        if stop_metric_new == "precision":
            return prec
        elif stop_metric_new == "recall":
            return rec
        elif stop_metric_new == "f1":
            return f1
        else:
            raise Exception("Wrong stop metric!!")

    def test_new_common(self, method, ids_1, ids_2, embeds1, embeds2, csls):
        embeds1 = torch.from_numpy(embeds1).float().to(self.torch_device)
        embeds2 = torch.from_numpy(embeds2).float().to(self.torch_device)
        align_kg1_kg2 = compute_best_alignment_one_side(embeds1, embeds2, method)
        align_kg2_kg1 = compute_best_alignment_one_side(embeds2, embeds1, method)
        ...
        print("\tRecall:", rec)
        print("\tF1:", f1)
        top_k_csls_kg1 = compute_top_k_avg_csls(embeds1, embeds2, method, csls).to(self.torch_device)
        top_k_csls_kg2 = compute_top_k_avg_csls(embeds2, embeds1, method, csls).to(self.torch_device)
        align_kg1_kg2_csls = compute_best_alignment_csls(embeds1, embeds2, method, top_k_csls_kg1, top_k_csls_kg2)
        align_kg2_kg1_csls = compute_best_alignment_csls(embeds2, embeds1, method, top_k_csls_kg2, top_k_csls_kg1)
        aligns_csls = compute_bidirectional_alignments(align_kg1_kg2_csls, align_kg2_kg1_csls, ids_1, ids_2)
        prec_csls, rec_csls, f1_csls = compute_prec_rec_f1(aligns_csls, self.kgs.test_links + self.kgs.valid_links)
        print("Final test result with csls:")
